...
Both bench.jl and test.jl discover models by scanning the `models/` directory for `.jl` files and `include()`-ing them.
...
3. To enable testing: add a `PARAM_MAPS` entry in test.jl mapping reference parameter names to Turing varnames
